...
generate_config() {
    ...
    if [[ "$CERT_MODE" == "selfsign" ]]; then
        ...
        LINK="hysteria2://${PASS}@${IP}:${PORT}/?obfs=salamander&obfs-password=${OBFS_PASS}&insecure=1&pinSHA256=${CERT_HASH}&sni=bing.com#HY2-${IP}"
        TLS_BLOCK="tls:
  sni: bing.com
  insecure: true
  pinSHA256: ${CERT_HASH}"
...
# 官方原始 unit 的 WorkingDirectory=~ 依赖 hysteria 用户的家目录，
# 但上面 useradd -r 未加 -m 不会创建家目录，且下面 ProtectHome=true
# 会屏蔽掉 /home 与 /root，两者叠加会导致服务 CHDIR 失败、反复重启崩溃。
# 显式指向证书/配置所在目录（已在 ReadWritePaths 中放行）以覆盖 ~。
